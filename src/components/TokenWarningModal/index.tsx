...
import { Button, Text } from '@flash-swap/uikit'
          ...
          {chainId && (
            <ExternalLink style={{ fontWeight: 400 }} href={getBscScanLink(chainId, token.address, 'token')}>
              <Text title={token.address}>
                {shortenAddress(token.address)} {TranslateString(116, '(View on Cronos Explorer)')}
              </Text>
            </ExternalLink>
          )}
        </AutoColumn>
      ...
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          ...
          <Text>
            {TranslateString(
              1130,
              'Anyone can create a CRC20 token on Cronos with any name, including creating fake versions of existing tokens and tokens that claim to represent projects that do not have a token.'
            )}
          </Text>
          <Text>
            {TranslateString(
              1132,
              'This interface can load arbitrary tokens by token addresses. Please take extra caution and do your research when interacting with arbitrary CRC20 tokens.'
            )}
          </Text>
